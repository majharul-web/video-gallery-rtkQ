...
export default function Description({ date, description, title, id }) {
  const [deleteVideo, { isError, isSuccess, isLoading }] = useDeleteVideoMutation();
    ...
    <div>
      ...
      <div className='pb-4 flex items-center space-between border-b gap-4'>
        ...
        <div className='flex gap-6 w-full justify-end'>
          ...
          <div className='flex gap-1 curser' onClick={handleDelete}>
            <div className='shrink-0'>
      ...
      <div className='mt-4 text-sm text-[#334155] dark:text-slate-400'>
        {isError && <Error message='Edit error' />}
      </div>
    </div>
  );
}
